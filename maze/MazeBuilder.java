...
//public class MazeBuilder implements Runnable {
	...
	public MazeBuilder(boolean deterministic){
		...
		if (true == deterministic)
			random.setSeed(5); //we simply seed the generator with some value.  This will produce the same 
							   //sequence of numbers
	}
...
/**
 * code to generate a solution to the maze.  This happens through random selection of positions in 
 * which to travel from the starting position.  An exit is  
 * 
 */
...
protected void setExitPosition(int remotex, int remotey) { 
	...
	protected void setStartPositionToCellWithMaxDistance() {
		...
		//finds the cell with the largest distance
		for (x = 0; x != width; x++)
			for (y = 0; y != height; y++) {
				if (dists[x][y] > d) {
					startx = x;
	...
	protected void computeDists(int ax, int ay) {
		...
		for (x = 0; x != width; x++)
			for (y = 0; y != height; y++)
				dists[x][y] = inf; //all cells have this value
	...
	private void genSegs() {
		...
		//for loop for the left and right borders
		for (y = 0; y != height; y++) {
			x = 0;
			while (x < width) {
				...
				//if there is a wall on top (i.e. the cell does not contain an exit)
				int startx = x;
		...
		//for loop for the the top and bottom borders 
		for (x = 0; x != width; x++) {
			y = 0;
			while (y < height) {
	 ...
	 */
	public void build(Maze mz, int w, int h, int roomct, int pc, boolean includeSeed) {
		...
		buildThread = new Thread(this); //here the new thread is created
		buildThread.start();
	}
	...
	public void run() {
		...
		maze.newMaze(root, cells, dists, startx, starty); //maze is of type Maze. 
	}
