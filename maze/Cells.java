...
public class Cells {
	...
	/*
	 * The following is a simple get method that will return the value stored in a given
	 * index
	 */
	
	public int returnValueAt (int x, int y){
		int value = cells[x][y];
	...
	public String toString() {
		String s = "" ;
		for (int i = 0 ; i < width ; i++) //the column variable
		{
			for (int j = 0 ; j < height ; j++)//the row variable
				s += " x:" + i + " y:" + j + "=" + cells[i][j] ; //i = x; j = y
			s += "\n" ;
		}
		return s ;
	}
