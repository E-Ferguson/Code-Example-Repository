...
public class Maze extends Panel {
	...
	public void newMaze(BSPNode root, Cells c, int dists[][], int startx, int starty) {
		...
		//dx = 1; dy = 0;
		setCurrentDirection(1, 0) ; //changed from (1, 0) -> to the right
		setCurrentPosition(0, 0) ; //here the code is changed!! to (0, 0)
		walk_step = 0;
		view_dx = dx<<16; 
		view_dy = dy<<16;
		ang = 0;
	...
	public void redraw() {
		...
		switch (state) {
		case STATE_TITLE:
                        System.out.println( "In STATE_TITLE" );
			redrawTitle(gc);
			break;
		case STATE_GENERATING:
                        System.out.println( "In STATE_GENERATING" );
			redrawGenerating(gc);
			break;
		case STATE_PLAY:
                        System.out.println( "In STATE_PLAY" );
			redrawPlay(gc);
			break;
		case STATE_FINISH:
                        System.out.println( "In STATE_FINISH" );
			redrawFinish(gc);
			break;
		}
		update(getGraphics());
	...
	synchronized protected void walk(int dir) {
		...
		/*System.out.println("The current cell: x = " + getCurrentPosition()[0] + " y = " + getCurrentPosition()[1] );
		
		System.out.println("\n The top bound: " + mazecells.hasMaskedBitsTrue(getCurrentPosition()[0], getCurrentPosition()[1], 32));
		System.out.println("The bottom bound: " + mazecells.hasMaskedBitsTrue(getCurrentPosition()[0], getCurrentPosition()[1], 64));
		System.out.println("The left bound: " + mazecells.hasMaskedBitsTrue(getCurrentPosition()[0], getCurrentPosition()[1], 128));
		System.out.println("The right bound: " + mazecells.hasMaskedBitsTrue(getCurrentPosition()[0], getCurrentPosition()[1], 256 )+"\n");
		*/
	...
	synchronized protected void rotate(int dir) {
		//System.out.println("before rotation: " + ang);
		int origang = ang; //modified in both this method and 
		int steps = 4;
		...
		//System.out.println("after rotation: "+ang + "\n");
		rotateFinish();
		System.out.println("The current direction: x = " + getCurrentDirection()[0] + " y = " + getCurrentDirection()[1] );
		//System.out.println("after rotation: "+ang + "\n");
	}
	...
	public void init() {
		...
		MazeBuilder mbjunk = new MazeBuilderPrim(); //newMazeBuilder
	...
	protected void build(int skill) {
		...
		percentdone = 0;
		redraw();
		mazebuilder = new MazeBuilderPrim();//changed from new MazeBuilder
		mazew = skill_x[skill];
		mazeh = skill_y[skill];
		int roomcount = skill_rooms[skill];
		mazebuilder.build(this, mazew, mazeh, roomcount, skill_partct[skill], false);
